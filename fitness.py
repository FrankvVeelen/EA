...
class Fitness:
    def __init__(self, fitness_type, size_population, num_objectives, weight_vector):
        ...
        self.fitnesses = [None]*num_objectives # fitnesses is array of fitness[objective][genotype]
        for i in range(self.num_objectives):
            self.fitnesses[i] = [None]*self.size_population

        self.best_genotype_1d = [inf] * num_objectives  # called z[i] in paper
                ...
                for city in population[genotype]:
                    if previousCity is not None:
                        ...
                        fitness += distance_between_nodes(problem.nodes[previousCity], problem.nodes[city],
                                                          objective)
                    previousCity = city
                self.fitnesses[objective][genotype] = fitness
    ...
    def calculate_z_optimums_pop(self):
        ...
        for objective in range(self.num_objectives):
            for genotype in range(self.size_population):
                if self.fitnesses[objective][genotype] < self.best_genotype_1d[objective]:
                    self.best_genotype_1d[objective] = self.fitnesses[objective][genotype]
    ...
    def calculate_z_optimums_genotype(self, genotype_fitness):
        ...
        for objective in range(self.num_objectives):
            print("fitness new child: " + str(genotype_fitness[objective]),
                  "old best: " + str(self.best_genotype_1d))
            if genotype_fitness[objective] < self.best_genotype_1d[objective]:
                self.best_genotype_1d[objective] = genotype_fitness[objective]
